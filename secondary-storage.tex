...
When we talk about computer secondary storage, we should briefly explain what computer secondary storage is. As the name suggest there also exists computer primary storage. This part of the memory hierarchy contains the CPU registers, CPU caches (cache levels one to three) and the System DRAM. It is directly accessible by CPU but volatile on power loss. That means that all data are lost if the power supply is interrupted or the computer is shut down. Computer secondary storage is in the memory hierarchy one layer below the System DRAM. It includes flash memory (like SSD, USB flash drives), optical discs (CD-ROM, DVD), magnetic discs (HDD, Floppy) and magnetic tape (Compact Cassette). Secondary storage is also known as external memory and differs from primary storage in that it is not directly accessible by the CPU. The computer usually uses its input/output channels to access secondary storage. It is non-volatile and keeps data on power loss. This report is about hard disk drives and solid state drives only.
\\
Hard disk drives use magnetic coated rotating disks (called 'platters') to store data. The direction of magnetization represents binary data bits. The actuator head detects and modifies the magnetization. The actuator arm moves to the data cylinder on the disc and waits at this position for data. (TODO: MORE EXPLANATION?) 
\\
Solid state drives use two key components: The controller and the memory. The controller is is an embedded processor and the connection between the host computer and the memory. It executes code on firmware-level; here are some of the functions performed by the controller: Error-correction code (ECC), Wear-leveling, garbage collection and encryption. The controller is very important for the performance of a SSD. If the controller provides multiple channels and can address all of them in parallel, there can be input/output operations simultaneously on different memory chips, what increases the speed and response time. The memory is of course the location where data are saved on and usually is DRAM or NAND flash memory. NAND is cheaper and non-volatile, but it is slower than DRAM. Most SSDs are used as long-time storages, so the non-volatile property is very important, which means that most SSDs use NAND flash memory. These memory chips can be organized as Single-level cell (SLC) or Multi-level cell (MLC). SLC is ten times more persistent and allows three times faster Sequential Write with nearly the same Sequential Read performance, but it is 30 % more expensive. The usage of SLC or MLC chips is mostly determined by pricing: Cheap SSDs use MLC chips, high performance SSDs SLC. (TODO: This section it cut down for any reason)

\section{History of storage drives}

\subsection{Hard Disk Drives}
Hard disk drives are the successor to drum memory. The first HDD was the IBM 350. It had a capacity of 5 MB and was as big as a locker, because it's actuator arm was moved electro-pneumatic. It was announced in 1956 and was not sold but rented for 650 dollar a month. Years later IBM started the Winchester project with the goal to develop a rotating memory with firmly mounted medium. The first HDD of the WInchester project was the IBM 3340 in 1979. The first Winchester drive in 5.25" form factor was the Seagate ST506 (in 1980). This is not only the name of this HDD but also the name of the interface. The ST506-interface established as de-facto-standard. In 1986 SCSI was specified. This was one of the first HDD protocols. Three years later ATA, which is another HDD interface, was specified. In 1997 Giant Magnetoresistive Effect (GMR) was used for the first time. This is a physically effect that appears within structures with alternately magnetic and non-magnetic layers and allowed to considerably increase the storage capacity. The First SATA-HDD used Native Command Queueing (NCQ) in 2004. NCQ is a technology to reorder the input/output operations and improve the performance. The first prototype of 2.5" Hybrid-HDD (H-HDD) included NAND flash memory and was announced in 2005. The flash memory was used as cache to enhance the speed and the response time of the HDD. The 1 Terabyte limit was reched by Hitachi in 2007, the 2 TB and 3 TB limit by Western Digital in 2009/2010. Important form factors of HDD are 5.25.", 3.5", 2.5" (used in laptops) and 1.8" (also used in laptops), important internal interfaces are ATA (IDE), ESDI, SCSI, SATA, Serial Attached SCSI (SAS) and ST506 (important external interfaces: FireWire, USB and eSATA). When there were multiple manufacturers on the HDD market for years, today only Seagate, Toshiba and Western Digital produces HDDs.
