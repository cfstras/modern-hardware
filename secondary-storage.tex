...
A Fractal Prefetching B$^{+}$-Tree \cite{chen2002fractal} is a single index structure that can be viewed at two different chunk sizes: outer chunks are sized to fit disk pages, while inner pages are sized to fit cache lines. This allows cache optimized access at two layers in the memory hierarchy at the same time, leading to an optimization of cache and I/O operations.
